...
**CI/CD:** `.github/workflows/tests.yml` runs `pytest` on every push/PR -- the general suite across Python 3.10 and 3.12, plus a separate job that runs the serving tests against the exact scikit-learn version `models/model.joblib` was pickled with (an unpinned sklearn resolves to a newer release on some matrix legs, and the pickle does not survive that). `.github/workflows/deploy.yml` builds `Dockerfile.serve` and rolls it out to an Azure Container App whenever the serving code or the committed model artifact changes on `master`, authenticating via OpenID Connect federated credentials -- no Azure client secret is stored in GitHub at all, and the Container App pulls from the registry with a user-assigned managed identity rather than registry credentials. `infra/main.bicep` provisions Log Analytics, Application Insights, an Azure Container Registry, and a Container Apps environment/app; `infra/setup.sh` is the one-time bootstrap that creates the resource group, deploys the Bicep template, and registers the federated credential. The service is deployed and live.
...
- *The Container App is created with its private image, not updated onto one.* This one cost two days, so it is worth writing down. `main.bicep` has to create the app on a public placeholder image, because on a first run nothing has been pushed to ACR yet. But Azure does not persist a registry entry for a registry the current image does not come from -- so the app is left with no registry binding, and the obvious next move (`az containerapp update --image <acr image>`) then fails to pull with a misleading `Authentication failed when pulling container image`. Worse, each failed provision leaves the resource harder to recover, and after enough of them no update of any kind succeeds: it has to be deleted and recreated. `deploy.yml` therefore creates the app outright -- private image and registry identity in a single request, the form Microsoft's own documentation prescribes -- when it does not yet exist, and only updates the image when it does. Note also that `az containerapp registry set --identity` silently does nothing when that identity is already assigned to the app, which makes this failure mode considerably harder to diagnose than it should be.
